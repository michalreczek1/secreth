  ...
  async function init() {
    await query(`
      ...
      CREATE TABLE IF NOT EXISTS rooms (
        ...
        bot_speed TEXT NOT NULL DEFAULT 'normal',
      ...
      ALTER TABLE rooms
      ADD COLUMN IF NOT EXISTS bot_speed TEXT NOT NULL DEFAULT 'normal';
  ...
  const store = {
    ...
    rooms: {
      ...
      async create(id, name, ownerId, ownerName, options = {}) {
        const { rows } = await query(`
          INSERT INTO rooms (id, name, owner_id, owner_name, state, bot_difficulty, bot_speed, game_data, created_at)
      ...
      async setBotSpeed(id, botSpeed) {
        await query('UPDATE rooms SET bot_speed = $2 WHERE id = $1', [id, botSpeed]);
      },
      async delete(id) {
